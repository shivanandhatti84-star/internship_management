...
const getStudentAssignmentHtml = (studentName, studentUsn, mentorName, mentorUsn, mentorEmail, company, redirectInfo) => {
  return `
    <div style="font-family: Arial, sans-serif; padding: 20px; color: #333; max-width: 600px; margin: auto; border: 1px solid #ddd; border-radius: 8px;">
      ...
      ${redirectInfo && redirectInfo.redirected ? `
      <p style="background-color: #fff3cd; color: #856404; padding: 10px; border-radius: 4px; font-size: 13px; margin-bottom: 15px;">
        <strong>[Sandbox Mode Alert]</strong> This email was originally sent to <strong>${redirectInfo.originalEmail}</strong>, but was redirected here to enable local testing.
      </p>` : ''}
      ...
      <p style="font-size: 12px; color: #777; text-align: center;">This is an automated notification from the Internship Management Portal.</p>
    </div>
  `;
};
...
const getEvaluationScheduledHtml = (studentName, mentorName, evaluationNumber, formattedDate, redirectInfo) => {
  return `
    <div style="font-family: Arial, sans-serif; padding: 20px; color: #333; max-width: 600px; margin: auto; border: 1px solid #ddd; border-radius: 8px;">
...
const sendEvaluationScheduledEmail = async ({
  ...
  if (smtpTransporter) {
    try {
      ...
      console.log(`[SMTP] Evaluation scheduled email sent to Student: ${studentEmail}`);
      return;
    } catch (err) {
